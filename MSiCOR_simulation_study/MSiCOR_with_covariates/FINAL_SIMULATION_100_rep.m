...
parfor zzzz = 1:num_simulations
    ...
    for ii = 1:max_runs
        ...
        for i=1:no_iter
            ...
            for j = 1:n
                ...
                [zzzz, ii,i,j,log(current_lh_1)/log(10), epsilon]
    ...
    for i = 1:N
        ...
        %%% Selection by prob draw %%%%%%%%%%%
        
        prob_vec = abs(Likelihood_covs_and_seqs/sum(Likelihood_covs_and_seqs));
        pdd = makedist('Multinomial','probabilities',prob_vec);
        rr = random(pdd);
        ppp = rr;
        %     [aa,bb] = max(membership_prob_TRUE(i,:));
        %     pp = bb;
    ...
    proportions_TRUE_from_cov_and_seq = zeros(K,1);
    for k = 1:K
        proportions_TRUE_from_cov_and_seq(k) = sum(TRUE_membership == k)/N;
    end
    
    filename = ['ZZZ_TRUE_proportions_',num2str(K),'_seed_',num2str(zzzz),'.csv'];
    csvwrite(filename,proportions_TRUE_from_cov_and_seq)
