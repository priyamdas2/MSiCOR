...
function value = objective_fun_identifiable_WITH_prop_CONTROLLED(theta,sequences,proportions)
    M = size(theta,2);
    K = size(theta,1)/(M+1);
    ...
    value = -estimate_likelihood_WITH_prop_CONTROLLED(matrices, initial_dists, sequences, proportions);
end
