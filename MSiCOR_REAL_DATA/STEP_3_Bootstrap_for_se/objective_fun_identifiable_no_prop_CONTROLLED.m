...
function value = objective_fun_identifiable_no_prop_CONTROLLED(theta,sequences)
    M = size(theta,2);
    K = size(theta,1)/(M+1);
    ...
    value = -estimate_likelihood_no_prop_CONTROLLED(matrices, initial_dists, sequences);
end
