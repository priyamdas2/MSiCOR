...
filename = ['MS_REAL_DATA_BETA_num_clus_',num2str(K),'.csv'];
...
proportions_EST_from_seq = zeros(K,1);
for k = 1:K
    proportions_EST_from_seq(k) = sum(max_index == k)/N;
end
proportions_EST_from_seq

%filename = ['MS_REAL_DATA_EXAMPLE_SEQ_PROPORTIONS_num_clus_',num2str(K),'.csv'];
%    csvwrite(filename,proportions_EST_from_seq)
    
    
%%%%%% Finding AIC BIC %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

num_parameters = K*M*(M-1) + K*(M-1) + p*(K-1);
loglikelihood = current_lh_1;
num_unique_patients = N;

AIC = 2*num_parameters - 2*loglikelihood;
BIC = num_parameters*log(num_unique_patients) - 2*loglikelihood;
    

filename = ['MS_REAL_DATA_AIC_BIC_num_clus_',num2str(K),'.csv'];
    csvwrite(filename,[AIC, BIC])
...
filename = ['MS_REAL_DATA_AIC_BIC_num_clus_',num2str(K),'.csv'];
loglike_AIC_BIC = csvread(filename);

AIC = loglike_AIC_BIC(1);
loglikelihood_correct = (AIC - 2*num_parameters)/2;
BIC_corrected = -2*loglikelihood_correct + log(N)*num_parameters;
AIC_corrected = -2*loglikelihood_correct + 2*num_parameters;

filename = ['MS_REAL_DATA_AIC_BIC_corrected_num_clus_',num2str(K),'.csv'];
    csvwrite(filename,[AIC_corrected, BIC_corrected])
