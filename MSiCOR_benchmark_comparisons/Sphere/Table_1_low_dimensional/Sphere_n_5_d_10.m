...
fun = @(x) sum(sum(((M-1)*x(1:(M-1),:).*(ub_here-lb_here)+lb_here).^2));
...
fun_ga = @(xxx) sum(((M-1)*xxx.*(ub_here_ga-lb_here_ga)+lb_here_ga).^2);
...
filename = ['Sphere_output_MSiCOR_IP_SQP_GA_d_', num2str(M-1),'_n_' ,num2str(n),'.xls'];
