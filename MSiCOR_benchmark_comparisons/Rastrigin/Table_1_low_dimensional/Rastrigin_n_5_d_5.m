...
lb_here = (-5.12)*ones(M-1,n);
ub_here = (5.12)*ones(M-1,n);


fun = @(x) (10*(M-1)*n+sum(sum(((M-1)*x(1:(M-1),:).*(ub_here-lb_here)+lb_here).^2-10*cos(2*pi*((M-1)*x(1:(M-1),:).*(ub_here-lb_here)+lb_here)),2)));

lb_here_ga = (-5.12)*ones(1,n*(M-1));
ub_here_ga = (5.12)*ones(1,n*(M-1));

fun_ga_rast = @(xxx) (10*(M-1)*n+sum(((M-1)*xxx.*(ub_here_ga-lb_here_ga)+lb_here_ga).^2-10*cos(2*pi*((M-1)*xxx.*(ub_here_ga-lb_here_ga)+lb_here_ga))));
...
for jjj = 1: sim_reps
    ...
    x_ga = ga(fun_ga_rast,size(X0,2),A,b,Aeq,beq,lb,ub,[],options);
    ga_times(jjj) = toc;
    ga_values(jjj) = fun_ga_rast(x_ga);
...
filename = ['Rastrigin_output_MSiCOR_IP_SQP_GA_d_', num2str(M-1),'_n_' ,num2str(n),'.xls'];
